...
| R4.3 | 4 | Podman floor (5, 4, 0) and no ceiling: nothing at or above the floor is refused; acceptance is silent within the qualified major and warns to stderr above it (backend-private `_version_qualified`, not part of the protocol surface); `JMS_RUNTIME_ACCEPT` ignored on Podman, warning included | unit | `test_podman_version_floor` |
